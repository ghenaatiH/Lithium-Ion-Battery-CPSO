...
for it=1:MaxIt
    ...
    w=w*wdamp;
    save('Temp_Result_with_top_JREE');
...
ChargingPro_25_F11=zeros(size(FadingRate25_Exp));
ChargingPro_35_F11=zeros(size(FadingRate35_Exp));
ChargingPro_45_F11=zeros(size(FadingRate45_Exp));
TopCost_25_F11=zeros(size(FadingRate25_Exp));
TopCost_35_F11=zeros(size(FadingRate35_Exp));
TopCost_45_F11=zeros(size(FadingRate45_Exp));
Predictionbuffer_25_F11=zeros(size(FadingRate25_Exp));
Predictionbuffer_35_F11=zeros(size(FadingRate35_Exp));
Predictionbuffer_45_F11=zeros(size(FadingRate45_Exp));
...
for N_cyc=1:numel(Predictionbuffer_25_F11)
    T=T25;
    ChargingPro_25_F11(N_cyc)= x(1)*(Ich^x(2))*(Idis^x(3))*exp(x(4)/T)*( N_cyc^(x(5)*(Ich^x(6))*(Idis^x(7))*exp(x(8)/T)) );
    TopCost_25_F11(N_cyc)=x(9)*exp(x(10)/T)*Top(N_cyc)^x(11);
    Predictionbuffer_25_F11(N_cyc) = ChargingPro_25_F11(N_cyc) + TopCost_25_F11(N_cyc);
    T=T35;
    ChargingPro_35_F11(N_cyc)= x(1)*(Ich^x(2))*(Idis^x(3))*exp(x(4)/T)*( N_cyc^(x(5)*(Ich^x(6))*(Idis^x(7))*exp(x(8)/T)) );
    TopCost_35_F11(N_cyc)=x(9)*exp(x(10)/T)*Top(N_cyc)^x(11);
    Predictionbuffer_35_F11(N_cyc) = ChargingPro_35_F11(N_cyc) + TopCost_35_F11(N_cyc);
    T=T45;
    ChargingPro_45_F11(N_cyc)= x(1)*(Ich^x(2))*(Idis^x(3))*exp(x(4)/T)*( N_cyc^(x(5)*(Ich^x(6))*(Idis^x(7))*exp(x(8)/T)) );
    TopCost_45_F11(N_cyc)=x(9)*exp(x(10)/T)*Top(N_cyc)^x(11);
    Predictionbuffer_45_F11(N_cyc) = ChargingPro_45_F11(N_cyc) + TopCost_45_F11(N_cyc);
    
end

absErr25_F11 = abs(FadingRate25_Exp-Predictionbuffer_25_F11)';
absErr35_F11 = abs(FadingRate35_Exp-Predictionbuffer_35_F11)';
absErr45_F11 = abs(FadingRate45_Exp-Predictionbuffer_45_F11)';
...
absErr_F11=[absErr25_F11;absErr35_F11;absErr45_F11];
...
plot(1:180,absErr_F11,'r');legend('absErr JREE','absErr F11');
xlabel('N cyc');ylabel('Absolute Error');
ErrJREE=sum(absErrJREE)
ErrF11=sum(absErr_F11)

figure;
plot(1:60,FadingRate25_Exp,'*b');title('T25');hold on;
plot(1:60,FadingRateEQ_25,'*r');
plot(1:60,Predictionbuffer_25_F11,'*g');
plot(1:60,ChargingPro_25_F11,'-.k');
plot(1:60,TopCost_25_F11,'--m');
legend('T25 Exp','T25 JREE','T25 F11','ChargingPro 25','TopCost 25');xlabel('N cyc');ylabel('Fading Rate %');

figure;
plot(1:60,FadingRate35_Exp,'*b');title('T35');hold on;
plot(1:60,FadingRateEQ_35,'*r');
plot(1:60,Predictionbuffer_35_F11,'*g');
plot(1:60,ChargingPro_35_F11,'-.k');
plot(1:60,TopCost_35_F11,'--m');
legend('T35 Exp','T35 JREE','T35 F11','ChargingPro 35','TopCost 35');xlabel('N cyc');ylabel('Fading Rate %');

figure;
plot(1:60,FadingRate45_Exp,'*b');title('T45');hold on;
plot(1:60,FadingRateEQ_45,'*r');
plot(1:60,Predictionbuffer_45_F11,'*g');
plot(1:60,ChargingPro_45_F11,'-.k');
plot(1:60,TopCost_45_F11,'--m');
legend('T45 Exp','T45 JREE','T45 F11','ChargingPro 45','TopCost 45');xlabel('N cyc');ylabel('Fading Rate %');
...
FinalResultx_F11=InternalBestSol.Sol.x;
FinalResult_F11=InternalBestSol.Sol
save('Result_JREE_F11_cpso_4');
